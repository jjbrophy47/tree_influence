...
def process(args, exp_dir, out_dir, logger):
    ...
    for method, res in res_list:
        ...
        ax = axs[0]
        ax.errorbar(res['edit_frac'] * 100, res['loss'], linestyle=line[method],
        ...
        ax = axs[1]
        ax.errorbar(res['edit_frac'] * 100, res['acc'], linestyle=line[method],
                    color=color[method], label=label[method])
        ...
        ax = axs[2]
        ax.errorbar(res['edit_frac'] * 100, res['auc'], linestyle=line[method],
                    color=color[method], label=label[method])
...
if __name__ == '__main__':
    main(post_args.get_label_set_args().parse_args())
