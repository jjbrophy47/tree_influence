"""
Compute min. counterfactual influence set.
"""
import os
import sys
import time
...
def flip_labels(y, flip_idxs, objective):
    """
    Flip the labels in y for `flip_idxs` indices.
    """
    ...
    for flip_idx in flip_idxs:
        new_y[flip_idx] = 1 if y[flip_idx] == 0 else 0
...
def remove_and_evaluate(inf_obj, objective, ranking, tree,
    ...
    for i in range(1, X_train.shape[0]):
        ...
        flip_idxs = ranking[:i]
        new_y_train = flip_labels(y_train, flip_idxs, objective)
        ...
        if objective == 'binary' and len(np.unique(new_y_train)) == 1:
            ...
            prefix = f'{i:>5}: {i / X_train.shape[0] * 100:>5.3f}%'
            res = eval_fn(objective, new_tree, X_test, y_test, logger, prefix=prefix)

            new_label = np.argmax(res['pred'])
