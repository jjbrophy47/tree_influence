...
def main(test_size=0.2, seed=1):
    ...
    label_df = pd.read_csv('german.data', sep=' ', header=None)
    print('\ntime to read in data...{:.3f}s'.format(time.time() - start))

    # add label to processed dataset
    df['risk'] = label_df[20].apply(lambda x: 1 if x == 2 else 0)  # 0 - low risk, 1 - high risk
    ...
    features = {}
    features['label'] = ['risk']
    features['numeric'] = ['Age', 'Credit amount', 'Duration']
