...
def main(test_size=0.2, seed=1):
    ...
    # retrieve dataset
    start = time.time()
    df = pd.read_csv('UCI CBM Dataset/data.txt', sep='   ', header=None)
    print('\ntime to read in data...{:.3f}s'.format(time.time() - start))

    df.columns = ['lever_pos', 'ship_speed', 'gas_shaft_torque', 'gas_turbine_rpm', 'gas_gen_rpm',
                  'starboard_prop_torque', 'port_prop_torque', 'hp_turbine_exit_temp',
                  'gt_compressor_inlet_air_temp', 'gt_compressor_outlet_air_temp', 'hp_turbine_exit_pressure',
                  'gt_compressor_inlet_air_pressure', 'gt_compressor_outlet_air_pressure',
                  'gas_turbine_exhaust_gas_pressure', 'turbine_inject_control', 'fuel_flow',
                  'gt_compressor_decay_state', 'gt_turbine_decay_state']
    ...
    features = {}
    features['label'] = ['gt_turbine_decay_state']
    features['numeric'] = ['lever_pos', 'ship_speed', 'gas_shaft_torque', 'gas_turbine_rpm', 'gas_gen_rpm',
                           'starboard_prop_torque', 'port_prop_torque', 'hp_turbine_exit_temp',
                           'gt_compressor_inlet_air_temp', 'gt_compressor_outlet_air_temp', 'hp_turbine_exit_pressure',
                           'gt_compressor_inlet_air_pressure', 'gt_compressor_outlet_air_pressure',
                           'gas_turbine_exhaust_gas_pressure', 'turbine_inject_control', 'fuel_flow',
                           'gt_compressor_decay_state']
