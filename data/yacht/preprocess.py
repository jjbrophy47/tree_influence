...
def main(test_size=0.2, seed=1):
    ...
    # retrieve dataset
    start = time.time()
    df = pd.read_csv('yacht_hydrodynamics.data', delim_whitespace=True, header=None)
    print('\ntime to read in data...{:.3f}s'.format(time.time() - start))

    df.columns = ['longitude', 'prismatic_coef', 'len_displacement_ratio',
                  'beam_drought_ratio', 'length_beam_ratio', 'froude_number',
    ...
    features = {}
    features['label'] = ['residuary_resistance']
    features['numeric'] = ['longitude', 'prismatic_coef', 'len_displacement_ratio',
                           'beam_drought_ratio', 'length_beam_ratio', 'froude_number']
