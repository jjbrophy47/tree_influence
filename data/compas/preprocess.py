...
def main(test_size=0.2, seed=1):
    ...
    # retrieve dataset
    start = time.time()
    fp = 'propublicaCompassRecividism_data_fairml.csv/propublica_data_for_fairml.csv'
    df = pd.read_csv(fp)
    print('\ntime to read in data...{:.3f}s'.format(time.time() - start))

    # split data into train and test
    train_df, test_df = train_test_split(df, test_size=test_size,
                                         random_state=seed, stratify=df['score_factor'])
    ...
    features = {}
    features['label'] = ['score_factor']
    features['numeric'] = ['Two_yr_Recidivism', 'Number_of_Priors', 'Age_Above_FourtyFive',
